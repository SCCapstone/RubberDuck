...
def highScoreScreen(noises):
    ...
    subtitle_text_image = subtitleFont.render(
        "High Score Board", True, values.COLOR_Purple)
    ...
    play_again_text_image = subtitleFont.render(
        "Play Again", True, values.COLOR_Pink)
    ...
    screen.blit(
        play_again_text_image,
        (playAgainCords[0] +
         widthButton /
         2 -
         play_again_text_image.get_width() /
         2,
         playAgainCords[1] +
         25 -
         play_again_text_image.get_height() /
         2))
    screen.blit(
        home_text_image,
        (homeCords[0] +
    ...
    for event in pygame.event.get():
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                ...
                )[0] < playAgainCords[0] + widthButton and playAgainCords[1] < pygame.mouse.get_pos()[1] < playAgainCords[1] + 50:
                    # share stats
